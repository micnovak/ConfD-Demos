...
In case we want to run adapter against ConfD (_api_ mode), we can use `Makefile` `start` target to start ConfD with initial sample configuration.

.rebuild everything and start ConfD and load sample configuration
[source, shell, role="acopy"]
----
...
In the second case (data is provided by the data provider) the ConfD cannot
